...
class RumorDataset(Dataset):
    ...
    同时加载文本+图像特征+graph特征：
      ...
      - 读取 graph 特征 (graph_loader.get_graph_feature)
      - label
    """
    def __init__(self, data_items_dict, queries_root,
                 image_feat_dict, graph_feat_dict):
        super().__init__()
        ...
        self.graph_feat_dict = graph_feat_dict
    ...
    def __getitem__(self, idx):
        ...
        # Graph 特征 (key 与 graph_feat_dict 中的索引对应)
        # 这里假设 key 就是字符串 "0","1","2",... 跟保存时一致
        graph_feat = graph_loader.get_graph_feature(self.graph_feat_dict, key)
...
def collate_fn(batch):
    ...
    for s in batch:
        ...
        graph_feats.append(s["graph_feat"])
        bkeys.append(s["unique_key"])

    labels = torch.stack(labels, dim=0)          # [B]
    img_feats = torch.stack(img_feats, dim=0)    # [B, ???]
    graph_feats = torch.stack(graph_feats, dim=0)# [B, ???]
    return labels, texts, img_feats, graph_feats, bkeys


##############################################################################
# 3) build_dataloader
##############################################################################
def build_dataloader(data_items_dict, image_feat_dict, graph_feat_dict,
                     split='train', batch_size=16):
    ds = RumorDataset(
        data_items_dict=data_items_dict,
        queries_root=config.queries_root,
        image_feat_dict=image_feat_dict,
        graph_feat_dict=graph_feat_dict
    )
    shuffle_flag = (split == 'train')  # 训练集需要 shuffle
...
def evaluate(model, data_loader, criterion, device):
    ...
    for batch in data_loader:
        labels, texts, img_feats, graph_feats, bkeys = batch
        labels = labels.to(device)
        img_feats = img_feats.to(device)
        graph_feats = graph_feats.to(device)

        logits = model(texts, img_feats, graph_feats=graph_feats)
...
def evaluate_best_and_save(model, test_dl, device, model_tag, monitor=None):
    ...
    # 5) 更新 best_results.csv（只在新 F1 更高时）
    ...
    if len(row_idx) > 0:
        ...
        if f1_macro > old_f1:
            df_old = _update_or_insert_model_row(
                df_old, model_tag, acc, prec_macro, rec_macro,
                f1_macro, cm, avg_gpu_util, avg_gpu_mem, avg_sps, idx=row_idx[0]
            )
    else:
        df_old = _update_or_insert_model_row(
            df_old, model_tag, acc, prec_macro, rec_macro,
            f1_macro, cm, avg_gpu_util, avg_gpu_mem, avg_sps, idx=None
        )
...
def main(
    ...
    # 2.1) 加载 train/val/test 的 graph 特征 (新增部分)
    graph_train_path = os.path.join(config.saved_npy_folder, "graph_features_train.npy")
    graph_val_path   = os.path.join(config.saved_npy_folder, "graph_features_val.npy")
    graph_test_path  = os.path.join(config.saved_npy_folder, "graph_features_test.npy")

    graph_feat_dict_train = graph_loader.load_graph_features(graph_train_path)
    graph_feat_dict_val   = graph_loader.load_graph_features(graph_val_path)
    graph_feat_dict_test  = graph_loader.load_graph_features(graph_test_path)

    # 3) 构建 DataLoader
    train_dl = build_dataloader(
        data_items_train, train_feat_dict, graph_feat_dict_train,
        split='train', batch_size=16
    )
    val_dl = build_dataloader(
        data_items_val, val_feat_dict, graph_feat_dict_val,
        split='val', batch_size=16
    )
    test_dl = build_dataloader(
        data_items_test, test_feat_dict, graph_feat_dict_test,
